...
interface AppHeaderProps {
    ...
    // --- Layout Control ---
    alignTitle?: 'left' | 'center'; // ✅ New Prop to control alignment
...
const AppHeader: React.FC<AppHeaderProps> = ({
    title,
    subtitle,
    alignTitle = 'center', // Default is center
    showBack = true,
    leftIcon,
    onLeftPress,
    rightIcon,
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.container}>
                ...
                {/* --- LEFT CONTAINER --- */}
                {/* Only show container if there is something to show, otherwise collapse width if aligning left */}
                <View style={[styles.leftContainer, alignTitle === 'left' && !showBack && !leftIcon ? { width: 0 } : {}]}>
                    {showBack ? (
                        <TouchableOpacity
                            onPress={handleSafeBack}
                            style={styles.iconButton}
                        ...
                        // If aligning center, keep space. If left, remove space.
                        alignTitle === 'center' ? <View style={{ width: 24 }} /> : null
                    )}
                </View>
                ...
                <View style={[
                    ...
                    alignTitle === 'left' ? { alignItems: 'flex-start', paddingLeft: 16 } : { alignItems: 'center' }
                ]}>
                    {title && (
                        <View style={[styles.titleWrapper, alignTitle === 'left' ? { alignItems: 'flex-start' } : { alignItems: 'center' }]}>
                            <Text style={styles.title} numberOfLines={1}>
                                {title}
                            </Text>
                            {subtitle ? (
