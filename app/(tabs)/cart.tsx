...
export async function updateCartItem(userUUID: string, productId: string, quantity: number) {
  const res: any = await api(`/users/cart/patch/${userUUID}`, "PATCH", { productId, quantity });
  return res;
}
  ...
  const handleUpdateQuantity = async (cartId: string, productId: string, currentQty: number, change: number) => {
    ...
    try {
      if (user?.userUUID) {
        ...
        if (res.success) {
          setCarts(prevCarts => {
            if (res.isEmpty) return prevCarts.filter(c => c._id !== cartId);
            return prevCarts.map(c => c._id === cartId ? res.cart : c);
          });
        }
      }
          ...
          <View key={cart._id} style={styles.cartCard}>
            ...
            <View style={styles.shopHeader}>
              <View style={styles.shopInfo}>
                ...
                <View>
                  <Text style={styles.shopName}>Order #{index + 1}</Text>
                  <Text style={styles.shopSubtext}>{cart.items.length} Items</Text>
                </View>
              </View>
            </View>
            ...
            <View style={styles.billContainer}>
              ...
              <View style={styles.billRow}>
                ...
                <Text style={styles.billValue}>₹{cart.tax.toFixed(2)}</Text>
              </View>

              <View style={styles.billRow}>
                <Text style={styles.billLabel}>Delivery Fee</Text>
                <Text style={[styles.billValue, cart.deliveryFee === 0 && { color: '#2ECC71' }]}>
              ...
              <View style={styles.billRow}>
                ...
                <Text style={styles.billValue}>₹{cart.platformFee.toFixed(2)}</Text>
              </View>
              ...
              <View style={styles.totalRow}>
                ...
                <Text style={styles.totalValue}>₹{Math.round(cart.total).toFixed(2)}</Text>
              </View>
            </View>
            ...
            <TouchableOpacity
              ...
              onPress={() => router.push({ pathname: "/user/dashboard/checkout", params: { cartId: cart._id } })}
            >
              <Text style={styles.checkoutText}>Proceed to Pay ₹{Math.round(cart.total)}</Text>
              <Ionicons name="arrow-forward" size={18} color="#FFF" />
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  shopName: { fontSize: 16, fontWeight: '700', color: '#333' },
  shopSubtext: { fontSize: 12, color: '#888' },
  ...
  billTitle: { fontSize: 14, fontWeight: '700', marginBottom: 10, color: '#333' },
  ...
  // Checkout Button
  checkoutBtn: {
    backgroundColor: '#2ECC71', flexDirection: 'row', justifyContent: 'center', alignItems: 'center',
    paddingVertical: 14, borderRadius: 12, marginTop: 5,
    shadowColor: "#2ECC71", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, elevation: 3
  },
  checkoutText: { color: '#FFF', fontWeight: '700', fontSize: 16, marginRight: 8 }
});